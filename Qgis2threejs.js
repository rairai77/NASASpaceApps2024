      ...
      start: function () {
        ...
        this.keyframeGroups.forEach(function (group) {
          ...
          var showNBox = function (idx) {
            ...
            if (n && e) {
              if (currentNarElem) {
                currentNarElem.classList.add("invisible");
                currentNarElem.classList.remove("visible");
              }

              currentNarElem = E(n.id);
              if (currentNarElem) {
                currentNarElem.classList.remove("invisible");
...
class Q3DDEMLayer extends Q3DMapLayer {
  ...
  getZ(x, y) {
    for (var i = 0, l = this.blocks.length; i < l; i++) {
      ...
      if (sdx <= 1 - sdy)
        return z[0] + (z[1] - z[0]) * sdx + (z[2] - z[0]) * sdy;
...
class Q3DPointLayer extends Q3DVectorLayer {
  ...
  buildLabels(features) {
    super.buildLabels(features, function (f) {
      return f.geom.pts;
    });
  }
}
...
class Q3DLineLayer extends Q3DVectorLayer {
  ...
  build(features, startIndex) {
    if (this._lastObjType !== this.properties.objType)
      this._createObject = null;
...
class Q3DPolygonLayer extends Q3DVectorLayer {
  ...
  build(features, startIndex) {
    if (this.properties.objType !== this._lastObjType)
      this._createObject = null;
